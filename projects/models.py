...
class Project(models.Model):
    ...
    def calculate_score(self):
        filled_fields = [self.total_area, self.legal_reserve_area, self.address, self.status_car,
                         self.sicar_code, self.matricula_status, self.georeferencing_status,
                         self.reserve_legal_status, self.physical_or_legal_entity, self.conservation_unit,
                         self.owner_actions_to_preserve_forest, self.legal_reserve_deficit, self.has_federal_debt]
        score = sum(1 for field in filled_fields if field)
        score += 3 * sum(1 for field in [self.pdf_matricula_certificate, self.pdf_car,
                                         self.property_polygon, self.pdf_federal_debt_certificate,
                                         self.pdf_ccir] if field)
        return score
